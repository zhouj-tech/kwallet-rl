...
def build_paths(config: Dict[str, Any]) -> Dict[str, str]:
    ...
    # =========================================================
    # Save all context-aware outputs under src/ideaextra/context_runs
    # =========================================================
    ideaextra_dir = PROJECT_ROOT / "src" / "ideaextra"

    C_value = int(config["env"]["C"]) if float(config["env"]["C"]).is_integer() else config["env"]["C"]
    seed_value = config["seed"]
    setting_tag = f"C{C_value}_seed{seed_value}"

    context_run_root = ideaextra_dir / "context_runs"

    result_scenario_dir = context_run_root / "results" / setting_tag / scenario
    result_run_dir = result_scenario_dir / run_stamp

    checkpoint_scenario_dir = context_run_root / "checkpoints" / setting_tag / scenario
