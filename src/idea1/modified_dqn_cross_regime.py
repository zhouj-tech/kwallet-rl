...
CONFIG = {
    ...
    # 新的数据配置：训练用一个 regime，测试时在多个 regime 上评估
    "data": {
        "train_regime": "D",
        "train_pool_file": "D_switching_quick_seed42.json",
        "test_pool_files": {
            "A": "A_regular_quick_seed42.json",
            "B": "B_heavytail_quick_seed42.json",
            "C": "C_burst_quick_seed42.json",
            "D": "D_switching_quick_seed42.json"
        }
    },
...
class KWalletEnv:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        oversize_dropped_this_step = False
        ...
        if tx > self.wallet_size:
            ...
            oversize_dropped_this_step = True
...
def train_agent(
    ...
    if config["save_mode"] == "full":
        torch.save(agent.q_net.state_dict(), paths["model_path"])
