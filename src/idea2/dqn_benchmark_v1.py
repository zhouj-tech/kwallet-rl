...
"""
benchmark_v1 最小 DQN 训练脚本
======================================================
当前目标：
1. 支持 specialist 模式：在单一 regime 上训练
2. 支持 generalist 模式：在 mixed regimes 上训练
3. 统一在 benchmark_v1 四类 regime 上评估
4. 输出简洁、关键、可复现的结果
...
后面再扩展到 SH / BL / BH specialist。

------------------------------------------------------
使用方法（先改下面 CONFIG）：
1. specialist:
   CONFIG["data"]["train_mode"] = "specialist"
   CONFIG["data"]["train_regime"] = "SL"

2. generalist:
   CONFIG["data"]["train_mode"] = "generalist"
   CONFIG["data"]["train_regime"] = None
------------------------------------------------------
"""
...
# ============================================================
# 1) 导入 benchmark_v1 generator
#    如果你后面复制成 benchmark_v1_generator.py，也能兼容
# ============================================================
try:
...
CONFIG = {
    ...
    # -------- 训练模式 --------
    "data": {
        # 可选: "specialist" / "generalist"
        "train_mode": "specialist",
        # specialist 时指定；generalist 时设为 None
        "train_regime": "SL",
        # 固定 benchmark_v1 的 regime 顺序
        "eval_regimes": ["SL", "SH", "BL", "BH"],
    },

    # -------- 环境参数：与 benchmark_v1 保持一致 --------
    "env": {
    "C": 3000.0,
    "k": 3,
    "T": 1000,
    ...
    # -------- 训练参数 --------
    "train": {
        "episodes": 900,          # 调试先小一点；正式实验可升到 1000~3000
        "max_steps": 300,         # 调试先小一点；正式实验可升到 1000 或 2000
        "batch_size": 128,
        "gamma": 0.98,
        "lr": 1e-3,
        "buffer_size": 50000,
    ...
    "eval": {
        "num_episodes": 50,
        "max_steps": 300,         # 调试先与 train 一致；正式实验再升高
    },

    # -------- 输出 --------
    "output": {
        "save_results": True,
        "results_dir": "dqn_benchmark_v1_outputs",
    },
}
...
def build_run_name(config: Dict) -> str:
    ...
    env = config["env"]
    return f"{tag}_C{int(env['C'])}_k{env['k']}_T{env['T']}_F{env['F']}"


def choose_train_regime(config: Dict, episode_idx: int, rng: random.Random) -> str:
    """
    训练时决定当前 episode 用哪个 regime。
    - specialist: 固定单一 regime
    - generalist: 从四类中均匀随机抽一个
    """
    mode = config["data"]["train_mode"]
    if mode == "specialist":
...
# ============================================================
# 4) 环境定义：最小可运行版
#    动作空间：settle_choice × flush_mask = (k+1) * 2^k
# ============================================================
class KWalletEnv:
    """
    状态：
    ...
    动作：
    - settle_choice: 0..k-1 表示尝试放入某个钱包；k 表示不放（drop / observe）
    - flush_mask: 长度 k 的 bitmask，决定本步对哪些钱包执行 flush
    """
    ...
    def __init__(self, C: float, k: int, T: int, F: int, max_steps: int, reward_cfg: Dict):
        ...
        self.num_actions = (self.k + 1) * (2 ** self.k)
    ...
    def decode_action(self, action_idx: int) -> Tuple[int, int]:
        flush_mask = action_idx // (self.k + 1)
        settle_choice = action_idx % (self.k + 1)
        return settle_choice, flush_mask

    def _apply_flush_mask(self, flush_mask: int) -> int:
        flush_count = 0
        for i in range(self.k):
            if ((flush_mask >> i) & 1) == 1:
                # 允许对任意钱包发起 flush，统一在 F 步后恢复可用
                self.wallets[i] = self.wallet_size
                self.freeze_until[i] = self.t + self.F
                flush_count += 1
        self.num_flushes += flush_count
        return flush_count
    ...
    def step(self, action_idx: int) -> Tuple[np.ndarray, float, bool, Dict]:
        settle_choice, flush_mask = self.decode_action(action_idx)
        ...
        # 1) 先执行 flush
        flush_count = self._apply_flush_mask(flush_mask)
        info["flushes_this_step"] = flush_count
        ...
        if tx > self.wallet_size:
            ...
            if settle_choice < self.k:
                if self._usable(settle_choice) and self.wallets[settle_choice] >= tx:
                ...
                # settle_choice == k，表示当前不投放，直接 drop
                dropped = True
                self.num_drops += 1
                self.insufficient_drops += 1
                info["insufficient"] = True
        ...
        if self.reward_cfg["enable_shaping"]:
            reward -= self.reward_cfg["flush_cost"] * flush_count
            if dropped:
                reward -= self.reward_cfg["drop_penalty"]

            usable_wallets = [self.wallets[i] for i in range(self.k) if self._usable(i)]
            if len(usable_wallets) >= 2:
                imbalance = float(np.std(usable_wallets) / self.wallet_size)
                reward -= self.reward_cfg["imbalance_penalty"] * imbalance
