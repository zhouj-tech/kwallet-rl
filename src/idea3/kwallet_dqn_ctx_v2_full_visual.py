...
def evaluate_agent_on_array(
    ...
    """在已加载的二维 pool 上做轻量评估（用于 validation，不做完整打印）"""
...
def train_agent(
    ...
    # -----------------------------
    # 训练/验证数据准备
    # 1) 若显式给了 val_pool_file，则单独加载
    # 2) 否则从当前训练池尾部切出 val_split_episodes 条作为 validation
    # -----------------------------
...
def main():
    ...
    try:
        ...
        agent, returns, loss_history, epsilons, val_history = train_agent(
            config=CONFIG,
            paths=paths
        )
        ...
        if CONFIG["save_mode"] == "none":
            ...
            if comp_cfg.get("enabled", False):
                static_json = comp_cfg.get("static_results_json", "")
                ctx_json = comp_cfg.get("ctx_results_json", "")
                if static_json and ctx_json:
                    plot_ctx_vs_static_from_files(
                        ...
                        title=f"Ctx vs Static Comparison\n{paths['title_tag']}"
                    )
                else:
                    print("Comparison is enabled but result JSON paths are missing. Skip comparison plot.")
        ...
        if CONFIG["save_mode"] == "none":
            ...
            print(f"   - {paths['val_plot_path']}")
            print(f"   - {paths['val_history_json_path']}")
            print(f"   - {paths['eval_plot_path']}")
            if CONFIG.get("comparison", {}).get("enabled", False):
                print(f"   - {paths['compare_plot_path']}")
